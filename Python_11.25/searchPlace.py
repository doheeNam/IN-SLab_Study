...
seoul_id = "52744c4f56736b6135386672717a64"

naver_id = "qfgTiwqfG0AFOZBjxuPI"
naver_secret = "mC65qWmxF7"
naver_url = "https://github/doheeNam"
crs_code = "NHN:128"

URL_STATION = "http://openapi.seoul.go.kr:8088/"
URL_LOCAL = "https://openapi.naver.com/v1/search/local?query="
URL_STATICMAP = "https://openapi.naver.com/v1/map/staticmap.bin?"
...
def map_service(point_x, point_y):
    if point_x == 0 and point_y == 0:
        ...
        url = URL_STATICMAP
        url += "clientId="+naver_id
        url += "&url="+naver_url
        url += "&center="+str(point_x)+","+str(point_y)
        url += "&markers="+str(point_x)+","+str(point_y)+"&crs="+crs_code
        url += "&level=12&w=700&h=250&baselayer=default"
        ...
        if(rescode==200):
           ...
           if js:
               ...
               for element in js["items"]:
                   treePlace.insert("", 'end', iid=None, text=element["title"], values=[element["category"], element["telephone"], element["address"], element["description"], element["mapx"], element["mapy"]])
